...
public class CardGroupScript : GameGenericMBScript<CardScript>, YesOrNoInterface
{
    ...
    void Update()
    {
        ...
        if(_gameProperties.GetMeter().GetPercentage() == 100.0f && !_gameDone && _responseText.text == "")
        {
            WinGame();
        }
    }
    ...
    void InstantiateNormalCardsFunction(ref DefinitionClass _wordInput, ref Vector3 _posInput, ref int _numberInput)
    {
        ...
        if (_responseText != null) {
            ...
            _c1._text = _responseText;

            _c2._text = _responseText;
        }
    ...
    IEnumerator SetResponseTextC(string _textInput, Color _colorInput, Color _outlineColorInput, float _secondsToDisplayInput = 5.0f, bool _endGameInput = false)
    {
        ...
        _responseText.color = _colorInput;

        _responseText.gameObject.GetComponent<Outline>().effectColor = _outlineColorInput;
